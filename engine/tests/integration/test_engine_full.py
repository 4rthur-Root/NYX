...
"""Tests d'intégration — pipeline complet Dispatcher → State → RuleEngine → Alerter."""
import json
import time
import datetime
...
def engine_env(tmp_path):
    """Fixture créant un environnement complet avec base SQLite en fichier
    temporaire et dossiers de logs."""
...
def test_full_ssh_bruteforce(engine_env):
    ...
    for i in range(10):
        ...
        line = (
            f"{ts_str} debian-server sshd[123]: "
            "Failed password for root from 1.2.3.4 port 5555 ssh2"
        )
        dispatcher.dispatch(line, "srv-pme.log")
...
def test_full_smb_exfil(engine_env):
    ...
    line1 = (
        f"{ts_str_iso} OPNsense.internal filterlog[1]: "
        "76,,,uuid,vtnet1,match,block,in,4,0x0,,64,0,0,none,6,"
        "tcp,60,10.0.1.60,10.0.1.20,54321,22,0"
    )
    dispatcher.dispatch(line1, "OPNsense.internal.log")

    line2 = (
        f"{ts_str_iso} debian-server smbd[2]: "
        "admin read secret.txt from //direction from 10.0.1.60"
    )
    dispatcher.dispatch(line2, "srv-pme.log")
...
def test_full_kerberoasting(engine_env):
    ...
    for i in range(5):
        line = (
            f"{ts_str_iso} debian-server samba-audit[2]: "
            f'{{"Authentication": {{"eventId": 4769, '
            f'"remoteAddress": "ipv4:10.0.1.50:5610{i}", '
            f'"servicePrincipalName": "cifs/srv-{i}.nyx.tg", '
            f'"accountName": "employe"}}}}'
        )
        dispatcher.dispatch(line, "srv-pme.log")
